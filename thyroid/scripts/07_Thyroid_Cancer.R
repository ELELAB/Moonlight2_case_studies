# This script compare known thyroid cancer driver genes with moonlight findings.
...
GUST <- read.csv("../data/rawdata/GUST_THCA.csv")
data(NCG)

# Analysis -------------------------
Drivers <- Oncogenic_mediators_mutation_summary %>% filter( CScape_Driver >= 1)

## Extract NCG thyroid cancer candidates ----------------------
NCG %>% filter(str_detect(NCG_cancer_type,'thyroid')) %>% count()

## Oncogenic mediators which are NCG thyroid cancer candidates
breast_cand <- Oncogenic_mediators_mutation_summary %>% 
  filter(str_detect(NCG_cancer_type,'thyroid')) %>% pull(Hugo_Symbol)

## Driver mutations in above genes 
Drivers %>% 
  filter(Hugo_Symbol %in% breast_cand)

# GUST vs Moonlight Thyroid cancer ----------------------------
